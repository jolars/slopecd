...
\begin{problem}
  \min_{x \in \bbR^m}
  F \left(\sum_{i=1}^m \tilde{X}_{\mathcal{C}_i} x_i \right)
...
\eqref{pb:cluster-problem}. To do so, we will introduce the directional derivative for the
...
\[
  C(z) = \{j : |\beta(z)_j| = z\}.
\]
Next, for a  strictly decreasing sequence \(c_1, c_2, ..., c_m\) we let $\varepsilon_c$ be an arbitrary positive value such that the $c_i$'s are at least $\varepsilon_c$ apart:
% \begin{equation}
  % \label{eq:epsilon-c}
  $
  \big| c_i - c_j \big| > {\varepsilon_c}, \, \forall\, i \neq j \enspace.
  $
% \end{equation}
Then for %an \({\varepsilon_c}\) and
\(\delta \in \{-1, 1\}\)
% \mathurin{This can be hard to parse: $C$ implicitly depends on a vector that determines clusters. We can emphasize that  In all this section $\beta$ is fixed and we update only in the direction given by the sign of one cluster. Clusters $C_i$ and $c_i$ are fixed.}
% \JL{Yes, but I don't think there's any way around this since we must use this
%   construction in the definitions and proofs for the directional derivative and the
...
\begin{proof}
  ...
  \end{align}
  We have the following cases to consider: \(|z| = c_i\) for \(i \neq k\) \jw{I prefer \(|z| \in \{c_1,c_2,\ldots,c_m\} \), I don't like having a loose $i$ in $i\neq k$ } ,
  \(|z| = 0\), and \(z \notin \{0, c_i\}\) for \(i \neq k\).
...
For the case when $z=0$ there are two sub cases.
 ...
 The other sub case is when \(c_m \neq 0\). Here \eqref{eq:directional-derivative-sl1} reduces to
  ...
  \(|z| \notin \{0, c_i\}\) for \(i \neq k\) \jw{Same notational issue here}and, since since \(C(z +
...
\begin{proof}
  ...
  c_i\) for \(i \neq k\) or \(z = 0\) \jw{whenever \(|z| \in \{0,c_1,c_2,\ldots,c_m\}\)}. Let \(\gamma = \tilde{r}^T\tilde{x}\)
  ...
  \end{equation}
  \jw{Can we cite this conditions through the cases in $T_k$ then we don't need that many equations?}
