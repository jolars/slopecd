...
\begin{theorem}\label{thm:sl1-directional-derivative}
  ...
  \[
    ...
    \begin{cases}
      ...
      \sign(z)\delta\smashoperator{\sum_{j \in C(z + {\varepsilon_c} \delta)}} \lambda_{(j)^-_{z + {\varepsilon_c}\delta}} & \text{if } |z| = c^{(k)}_i > 0, \\
...
\begin{proof}
  ...
  \end{align}
  We have the following cases to consider: \(|z| = c_i^{(k)}\),
  \(|z| = 0\), and \(z \notin \{0, c_i^{(k)}\}\).
