...
To circumvent this issue, we combine these coordinate descent steps with full proximal gradient steps, which enable the algorithm to identify the cluster structure~\parencite{Liang2014} due to the partial smoothness property of the sorted \(\ell_1\) norm~(\Cref{app:sec:partly_smooth}).

\subsection{Hybrid Strategy}
\label{sec:hybrid-strategy}

We now present the proposed solver in \Cref{alg:hybrid}.
For the first and every $v$th iteration\footnote{Our experiments show that \(v\) has little impact on the speed of the algorithm as long as it is at least 3~(\Cref{sec:pgd-freq-study}). We have therefore set \(v = 5\) in our experiments.}, we perform a proximal gradient descent update.
...
\begin{algorithm}[h!]
  ...
  \If{\(t \bmod v = 0\)}{
    \(\beta \leftarrow \prox_{J}(\beta - \frac{1}{\lVert X \rVert^2}X^\top(X \beta - y); \lambda / \lVert X \rVert^2)\) \label{alg:hybrid-istastep}
...
\jl{This should be fine, it's exactly what we use in the code.}
...
\jl{I don't think we need this sentence, right?}
...
\begin{lemma}
  ...
  \[
    P(\beta^{(k)}) - P(\beta^*) \leq \frac{\Vert X \Vert^2 \lVert \beta^{(0)} - \beta^* \rVert^2}{2\lfloor k/v \rfloor }\, ,
  \]
...
In this paper we have only considered sorted \(\ell_1\)-penalized least squares regression.
In \Cref{sec:other-datafits}, we consider extensions to alternative datafits.
