...
\mm{Above F has been replaced by $\ell$, OK to change everywhere?}
\begin{problem}
...
First, let \(C(z)\) be the function that returns the cluster of $\beta(z)$ corresponding to \(z\), that is
\[
  C(z) = \{j : |\beta(z)_j| = z\} \,.
\]
\mm{add remark: it always contains $\mathcal{C}_k$, but it can contains mor if $z$ is equal to one of the other $c_i$'s ?}
...
\mm{Issue if $z$ is 0 in first equation? for $h \neq 0$ and $h = 0$ the argument of the limit jumps}
\begin{equation}
  \begin{aligned}
