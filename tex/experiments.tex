...
\begin{itemize}[noitemsep]
  ...
  \item The hybrid (our) solver (see \Cref{alg:hybrid}) combines proximal gradient descent
        ...
        We perform a coordinate descent step every fifth iteration~(\(v = 5\)) in the algorithm. (See \Cref{sec:hybrid-strategy}.)
...
\begin{figure*}[!t]
  ...
  \includegraphics{simulated.pdf} \caption{Benchmark on simulated datasets. The plots show suboptimality (difference between the objective at the current iterate, \(P(\beta)\), and the optimum, \(P(\beta^*)\)) as a function of time for SLOPE on multiple simulated datasets and $\lambda$ sequences of varying strength.}
  \label{fig:simulated}
\end{figure*}
...
\begin{figure*}[!t]
  ...
  \caption{Benchmark on real datasets. The plots show suboptimality (difference between the objective at the current iterate, \(P(\beta)\), and the optimum, \(P(\beta^*)\)) as a function of time for SLOPE on multiple simulated datasets and $\lambda$ sequences of varying strength.}
  \label{fig:real-data}
\end{figure*}
