...
Proximal coordinate descent cannot be applied to \Cref{pb:slope} because it is non-separable.
...
The combination of proximal gradient descent steps and proximal coordinate descent avoid the situation depicted in \Cref{fig:naive-cd-stuck} where coordinate descent gets stuck.

A full proximal coordinate descent may have been possible if one could decide wether a cluster should be split or not. 
However, the cost of checking the conditions under which a cluster splits is too high and the alternation between proximal gradient descent and proximal coordinate proved to be very efficient.
...
\jl{We should explain why we are not considering a pure CD algorithm, i.e. one that splits clusters directly.}
...
\begin{theorem}\label{thm:sl1-directional-derivative}
  ...
  \mathurin{move into notation section? it's not used in the theorem, only in the proof}
