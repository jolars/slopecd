...
\usepackage[orientation=landscape,size=custom,width=118.9,height=84.1,scale=1]{beamerposter}
	...
	\begin{block}{\textbf{\color{malgared}{\#3 Algorithm}}}
		...
		\begin{algorithm}[H]
			...
			\For{\(t \gets 0,1,\dots\)}{
			  ...
			  \If(\tcp*[h]{cluster identification step}){\(t \bmod v = 0\)}{
				\(\beta \leftarrow \prox_{\frac{1}{\norm{X}^2_2}  \, \text{slope}}\Big(\beta - \frac{1}{\norm{X}_2^2}X^T(X \beta - y)\Big)\) \label{alg:hybrid-istastep}

				Update \(c\), \(\mathcal{C}\)  \tcp{clusters and $\beta$ value on clusters}
			  }
			  \Else{
				\(k \gets 1\)

				\While(\tcp*[h]{fast coordinate descent steps}){\(k \leq \lvert \mathcal{C} \rvert\)}
				{
				  \(\tilde x_k \gets X_{\mathcal{C}_k} \text{sign}(\beta_{\cC_k}) \)

				  \(z \gets T(c_k\norm{\tilde x}^2 - \tilde x^T(X\beta - y); \norm{x}^2, c^{\setminus k}, \lambda)\) \tcp{SLOPE-thresholding operator}

				  $\beta_{\cC_k} \gets z \text{	sign}(\beta_{\cC_k})$
