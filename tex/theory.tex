...
\end{theorem}
\begin{proof}
  Let \(f(\tilde\beta) = |\tilde\beta|\sum_{j \in \mathcal{C}_i}\lambda_{(j)^-} + \sum_{j \notin \mathcal{C}_i} \lambda_{(j)^-}|\beta_j|\).
...
\[
  ...
  \frac{
    T \big(
    (\tilde y - y)^T X_{\mathcal{B}} s_{\mathcal{B}}, \lambda\big)
  }{
...
\begin{equation}
  ...
  \begin{cases}
    ...
    \sign(a)c_i                                              & \text{if } \sum_{j= |\mathcal{C}_i| - |\mathcal{B}|}^{|\mathcal{C}_i|} \lambda^{\mathcal{C}_i}_j \leq \big| |a| - c_i\big| \leq \sum_{j=1}^{|\mathcal{B}|}\lambda^{\mathcal{C}_i}_j \\
...
checking all possible combinations of a cluster likely is expensive. We have
