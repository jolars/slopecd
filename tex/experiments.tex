...
\begin{itemize}[noitemsep]
  ...
  We present in \Cref{fig:simulated_appendix} and \Cref{fig:real_appendix} a more detailed benchmarks for \texttt{ADMM} solvers with different values of this parameter and the adaptive setting. 
...
Our algorithm, in contrast, reaches convergence in only 2.91 seconds\footnote{Note also that we do not use any screening rule in the current implementation of our algorithm, unlike the \pkg{SLOPE} package, which uses the strong screening rule for slope~\parencite{larsson2020c}.}.
