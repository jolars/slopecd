...
First, as illustrated on \Cref{fig:partial_slope}, note that $H$ is piecewise affine, with breakpoints at 0 and all $\pm c_i$'s for which $i \neq k$.
...
+ p\log(p))$. Here $np$ is from matrix-vector multiplication and $p\log(p)$ is
for sorting the sorting in the prox-step. For the coordinate descent step the
worst case complexity is  $\mathcal{O}(np + m(m + n) )$. Now, this complexity is
something one does not see in practice. For instance, if all clusters are
constant and they retain they're ordering unchanged then the complexity, in our
implementation of algorithm \ref*{alg:hybrid}, is $\mathcal{O}(mn)$.
