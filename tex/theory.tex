...
\begin{theorem}
  ...
  \]
  \mm{cosmit: I use : instead of | in sets to avoid confusion with following absolute value}
  such that \(m\) is the number of clusters.
  \mathurin{to be sure: it's what we called $\cC_i$ above ? I tend to prefer C for clusters}
\end{theorem}
\begin{proof}
  The directional derivative for the sorted \(\ell_1\) norm and a direction
  \(v\) with \(\lVert v \rVert = 1\) \mm{is normalization needed?} is
  \begin{equation}
...
the relative signs that really matter, right?}
\[
