...
\mathurin{Maybe we need to motivate here: CD cannot be applied because the problem is not separable. However if one knows the clusters of the solution in advance, then the problem is the same as a separable Lasso where the clusters are collapsed [I'm not even sure I have a proof for that, but experimentally we have it]. So an idea is to try to identify the clusters, and to perform CD updates on the collapsed estimates.}
...
\[
  \begin{aligned}
    P(\beta) & =  \frac{1}{2} \lVert y - X\beta \rVert_2^2 + J(\beta)                                                                                                                                                                                                                \\
             & = \frac{1}{2} \lVert y - X_{\bar{\mathcal{C}}_k} \beta_{\bar{\mathcal{C}}_k} - \big(X_{\mathcal{C}_k} s_{\mathcal{C}_k}\big)c_k  \rVert_2^2
             + \sum_{j \notin {\mathcal{C}_k}} \lambda_{(j)^-}|\beta_k|
             + \bigg(\sum_{j \in {\mathcal{C}_k}} \lambda_{(j)^-}\bigg)c_k \\
             %
             & = \frac{1}{2} \lVert \tilde r - \tilde x_k c_k \rVert_2^2
             + \sum_{j \notin {\mathcal{C}_k}} \lambda_{(j)^-}|\beta_k|
             + \bigg(\sum_{j \in {\mathcal{C}_k}} \lambda_{(j)^-}\bigg)c_k
             \enspace , \\
  \end{aligned}
\]
with \( \tilde x_k = X_{\mathcal{C}_k} s_{\mathcal{C}_k} \in \mathbb{R}^n \) and
...
\begin{equation*}
  \minimize_{\alpha \in \mathbb{R}} \left( \frac{1}{2} \lVert \tilde r - \tilde x \alpha_k \rVert_2^2 + \sum_{j \notin {\mathcal{C}_k}} \lambda_{(j)^-}|\beta_k| + \bigg(\sum_{j \in {\mathcal{C}_k}} \lambda_{(j)^-}\bigg)|\alpha_k|\right) \enspace.
\end{equation*}
