...
We will now consider the variable $\tilde \beta_k$ which equals the magnitude of\(k\)th cluster, i.e. $|\beta_j|=\tilde \beta_k$ for all $j \in \mathcal{C}_k$.
...
such that \(\tilde r= y - \tilde y\) is the \emph{partial residual} and take the
...
\end{cases}
$$
.\mathurin{If $f$ is only a function of $\tilde \beta$, then the second term is constant, and since we are interested in subgradients, we should able to omit it?}
