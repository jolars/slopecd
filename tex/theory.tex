...
In this section we derive an expression for the partial \mm{how canon is this name?} subdifferential of the
...
the \(k\)th cluster. \mathurin{this may be ambiguous: I would look for a formula using ``in the direction of...''}
...
\mathurin{This can be hard to parse: $C$ implicitely depends on a vector that determiens clsuters. We can emphasize that  In all this section $\beta$ is fixed and we update only in the direction given by the sign of one cluster. Clusters $C_i$ and $c_i$ are fixed.}
...
\end{equation}
for \(\delta \in \{-1, 1\}\).
In other words, the order permutation corresponding to \(B(z + h\delta)\) depends only on \mm{typo B for $\beta$?}
