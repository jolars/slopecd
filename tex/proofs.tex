...
\begin{proof}
  ...
  Thus $J$ is linear on $\cM$ and 1 and 3 hold. \mathurin{maybe add a nitpick so that coefs don't cross 0, like condition on $\varepsilon_c$}

  If one cluster of $x$ vanishes ($c_m = 0)$, the same reasoning holds imposing that $u_{\cC_m} = 0$.

  \mm{tangent proof to finish}
\end{proof}