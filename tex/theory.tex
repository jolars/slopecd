...
\begin{problem}
\min_{z \in \bbR^{m^*}}
F \left(\sum_{i=1}^{m^*} \sum_{j \in \mathcal{C}_i^*} z_i \sign(\beta_j^*) e_j \right)
...
G(z) = P(\beta(z))  = F\big(\beta(z)\big) + H(z)
\Big) \,  ,
\end{problem}
where
\[
...
\begin{theorem}\label{thm:sl1-directional-derivative}
  ...
  \]
\end{theorem}
The proof is in \Cref{app:proof_directional_slope}.
...
\begin{figure}[htb]
  ...
  \(c^{\setminus k} = (2, 1)\). The solution of \Cref{pb:cluster-problem} is the value of \(z\) for
...
Since the regularization term for SLOPE is not separable, applying PCD does not guarantee convergence and PCD updates can be stuck at points that are the not the solution of \cref{pb:slope}.
However, \cite{dupuis2021} showed that once the clusters are known,  \cref{pb:slope} becomes separable with respect to the knowledge of the clusters of the solution $\hat{\beta}$.
Practically, it is impossible to know these clusters in advance but we propose to take advantage of the proximal gradient descent to play the role of making sure that we identify the right clusters throughout the iterative updates.

\Cref{alg:hybrid} presents the proposed algorithm. 
...
The combination of the proximal gradient steps and PCD allows us to overcome the problem of vanilla PCD getting stuck because of non-separability and allows us to enjoy the speed-up provided by making local updates on each cluster.
