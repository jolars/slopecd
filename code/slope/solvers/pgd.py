...
def prox_grad(
    ...
    for step in range(len(reg)):
        monitor = ConvergenceMonitor(
            X, y, alphas * reg[step], tol, gap_freq, max_time, verbose, False
        )
        ...
        if callback is not None:
            ...
            proceed = True
        while proceed:
            w_new = prox_slope(z + (X.T @ R) / (L * n_samples), alphas * reg[step] / L)
            if anderson:
                ...
                if it < K + 1:
                    ...
                    try:
                        ...
                        p_obj = norm(y - X @ w_new - intercept) ** 2 / (
                            2 * n_samples
                        ) + np.sum(alphas * reg[step] * np.sort(np.abs(w_new))[::-1])
                        p_obj_acc = norm(y - X @ w_acc - intercept) ** 2 / (
                            2 * n_samples
                        ) + np.sum(alphas * reg[step] * np.sort(np.abs(w_acc))[::-1])
                        if p_obj_acc < p_obj:
                            w_new = w_acc
