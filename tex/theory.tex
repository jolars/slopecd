...
\Cref{pb:slope}
\mathurin{Suggestion: \\
Coordinate descent cannot be applied to  because it is not separable.
However, if the clusters of the solution $\mathcal{C}_1, \ldots, \mathcal{C}_m$ were known together with its sign, then the values $c_1, \ldots, c_m$ taken by $\hat \beta$ on the clusters could be equivalently computed by solving :
\begin{problem}
 \min_{x \in \bbR^m}
...
First observe that we can write \Cref{pb:slope} as \mm{do we restrict to quadratic datafit? in the intro there's a generic df and our strategy would be adapted to it}
\[
  \begin{aligned}
    P(\beta)
