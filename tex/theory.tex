...
To circumvent this issue, we combine these coordinate descent steps with full proximal gradient steps~\parencite{bogdan2015}, which enable the algorithm to identify the cluster structure~\parencite{Liang2014} due to the partial smoothness property of the sorted \(\ell_1\) norm that we prove in \Cref{app:sec:partly_smooth}.
