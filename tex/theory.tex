...
\begin{definition} \mm{cosmetic: I'm not sure this is a definition}\jw{I agree with mm, and I prefer to have $\epsilon_c$ or for the clusters just use $C(z;\delta)=  \lim_{h \downarrow 0} \lambda_{(i)^-_{z + h\delta}}$ }
...
\jw{When is B used, if they are going to help understanding we need to use them}
\begin{remark}
...
\begin{theorem}\label{thm:sl1-directional-derivative}
  ...
  \[
    J'_k(z, \delta) =
    \begin{cases}
      \smashoperator[r]{\sum_{j \in C(\varepsilon \delta)}} \lambda_{(j)^-_{\varepsilon\delta}}                    & \text{if } z = 0,         \\
      \sign(z)\delta\smashoperator{\sum_{j \in C(z + \varepsilon \delta)}} \lambda_{(j)^-_{z + \varepsilon\delta}} & \text{if } |z| = c_i > 0, \\
...
\begin{proof}
  ...
  \end{align}
\jw{Add a sentence that you will go through the cases $z=0,|z|=c_i$ and otherwise in the order you go through the cases.}
  First, note that \(J_k\) is differentiable for \(z \notin \{0, c_i\}\), \(i
  \neq k\) and that \(C(z + \varepsilon\delta) = C(z)\)\jw{\(z \notin \{0, c_i\}\), \(i
  \neq k\) is strange I suggest using \(\{0, c_1,c_2,\ldots c_m\} \setminus c_k\). Then and is a consequence of the first argument}, giving
  ...
  simply,  since \(|\delta| = 1\) by definition, 
  \begin{equation*}
...
\jw{I would prefer to keep the if statement more similar, also otherwise is very easy so I would write out what that is  explicitly.}
  \[
    T_k(\gamma, \omega; c, \lambda) =
    \begin{cases}
      0                                                                                                                                           & \text{if } |\gamma| < \smashoperator{\sum_{j \in C(\varepsilon)}} \lambda_{(j)^-_\varepsilon},                                                                                                                                       \\
      \sign(\gamma)c_i                                                                                                                            & \text{if } \omega c_i + \smashoperator{\sum_{j \in C(c_i - \varepsilon)}} \lambda_{(j)^-_{c_i - \varepsilon}} \leq |\gamma| \leq \omega c_i + \smashoperator{\sum_{j \in C(c_i + \varepsilon)}} \lambda_{(j)^-_{c_i + \varepsilon}}, \\
      \frac{\sign(\gamma)}{\omega} \bigg( |\gamma| - \smashoperator{\sum_{j \in C(c_i + \varepsilon)}} \lambda_{(j)^-_{c_i + \varepsilon}} \bigg) & \text{if } c_i < \frac{1}{\omega} \bigg( |\gamma| - \smashoperator{\sum_{j \in C(c_i + \varepsilon)}} \lambda_{(j)^-_{c_i + \varepsilon}} \bigg) < c_{i -1},                                                                         \\
      \frac{\sign(\gamma)}{\omega} \bigg( |\gamma| - \smashoperator{\sum_{j \in C(c_1 + \varepsilon)}} \lambda_{(j)^-_{c_1 + \varepsilon}} \bigg) & \text{otherwise}
    \end{cases}
  \]
  with \(\varepsilon\) defined as in \cref{def:limit-permutation}. Then
  \(T(\tilde{r}^T\tilde{x}, \tilde{x}^T\tilde{x}; c; \lambda) \in \argmin_{z
  \in \mathbb{R}} J_k(z) \).\jw{We have room to define $\tilde{x}$, $\tilde{r}$}
\end{theorem}
\begin{proof}
\jw{Help the reader what you want todo. Also structure it clearer so it can be followed easier. Right now you start with $z$ conditions then move to $\gamma$ conditions.}
  ...
  continuous piecewise-differentiable function with kinks whenever \(z \in \{0, c_i\}\), \(i
  \neq k\). Let \(\gamma =
  ...
  \begin{equation}
    ...
    | \gamma | \leq \smashoperator{\sum_{j \in C(\varepsilon)}}\lambda_{(j)^-_{\varepsilon}}
  \end{equation}
  since \(C(\varepsilon) = C(-\varepsilon)\) and
  \(\lambda_{(j)^-_{-\varepsilon}} = \lambda_{(j)^-_{\varepsilon}}\).

  Next, \(z = c_i \neq 0\), \(i \neq k\) is a solution if and only if
  \begin{equation}
    \label{eq:case-cluster}
    \omega c_i + \smashoperator{\sum_{j \in C(c_i - \varepsilon)}} \lambda_{(j)^-_{c_i - \varepsilon}}
    \leq |\gamma| \leq
    \omega c_i + \smashoperator{\sum_{j \in C(c_i + \varepsilon)}} \lambda_{(j)^-_{c_i + \varepsilon}}
  \end{equation}
  since \(C(c_i + \varepsilon) = C(-c_i - \varepsilon)\) and
  \(C(-c_i + \varepsilon) = C(c_i - \varepsilon)\).
  ...
  \begin{equation}
    ...
    \omega c_i + \smashoperator{\sum_{j \in C(c_i + \varepsilon)}} \lambda_{(j)^-_{c_i + \varepsilon}}
    < |\gamma| <
    \omega c_{i - 1} + \smashoperator{\sum_{j \in C(c_{i - 1} - \varepsilon)}} \lambda_{(j)^-_{c_{i - 1} - \varepsilon}},
  \end{equation}
  for some \(i > 1\), or that
  \[
    |\gamma| > \omega c_1 + \smashoperator{\sum_{j \in C(c_1 + \varepsilon)}} \lambda_{(j)^-_{c_1 + \varepsilon}}.
  \]
  In the first case, note that
  \[
    \smashoperator{\sum_{j \in C(c_i + \varepsilon)}} \lambda_{(j)^-_{c_i + \varepsilon}}
    =
    \smashoperator[r]{\sum_{j \in C(c_{i-1} - \varepsilon)}} \lambda_{(j)^-_{c_{i-1} - \varepsilon}}
  \]
  and therefore \eqref{eq:differentiable-inequality} is equivalent to
  \[
    c_i < \frac{1}{\omega} \bigg( |\gamma| - \smashoperator{\sum_{j \in C(c_i + \varepsilon)}} \lambda_{(j)^-_{c_i + \varepsilon}} \bigg) < c_{i -1}.
  \]
  Now, for some \(z \in (c_i, c_{i-1})\), observe that
  \[
    \frac{1}{\omega} \bigg( |\gamma| - \smashoperator{\sum_{j \in C(c_i + \varepsilon)}} \lambda_{(j)^-_{c_i + \varepsilon}} \bigg)
    =
    \frac{1}{\omega} \bigg( |\gamma| - \smashoperator{\sum_{j \in C(z)}} \lambda_{(j)^-_{z}} \bigg),
  \]
  which is exactly the optimum for \(P_k(z)\) \jw{P?} since it is
  differentiable for \(|z| \in (c_i, c_{i -1})\) and \(|z| > c_1\) and has a
  unique minimizer
  \[
    \omega z - \gamma + \sign(z) \smashoperator{\sum_{j \in C(z)}} \lambda_{(j)^-_{z}} = 0
    \implies
    z = \frac{\sign(\gamma)}{\omega} \bigg( |\gamma| - \smashoperator{\sum_{j \in C(z)}} \lambda_{(j)^-_{z}} \bigg)
  \]
  \jw{To me the arrow should be in the opposite direction.}
  given that \(\sign(z) = \sign(\gamma)\) for there to exist a solution since \(\omega > 0\).

  The cases \(|z| > c_1\) and \(z \in (-c_{i-1}, c_i)\) follow analogously.
\end{proof}
