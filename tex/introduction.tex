...
\mathurin{Suggestion: replace up to `` current version of the SLOPE package.'' by: \\
This lackluster performance has hampered the applicability of SLOPE to many
real-world applications.
The main reason for SLOPE being slower to solve than the Lasso, MCP or SCAD is that its non-separability prevents it from being amenable to proximal coordinate descent.
Current solvers must resort to FISTA \cite{beck2009}, that in ML scenario is much slower than coordinate descent \cite{moreau2022benchopt}.
}
This lackluster performance has hampered the applicability of SLOPE to many
...
\begin{figure}[htbp]
  ...
  \caption{Illustration of proposed solver. \mathurin{to discuss with JL}}
