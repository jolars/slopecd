...
\[
  ...
  \end{aligned}
\]
Now let \(\tilde y = X_{\bar{\mathcal{C}}_k} \beta_{\bar{\mathcal{C}_k}}\), and $ \tilde x = X_{\mathcal{C}_k} s_{\mathcal{C}_k}$,
...
\begin{equation}
  ...
  \partial_{\tilde\beta}
  P(\beta) = (y - \tilde y)^T \tilde x + \tilde x^T \tilde x \tilde\beta + \partial_{\tilde\beta}\Bigg(\bigg(\sum_{j \in {\mathcal{C}_k}} \lambda_{(j)^-}\bigg)|\tilde\beta| + \sum_{j \notin \mathcal{C}_k}\lambda_{(j)^-}|\beta_k|\Bigg),
\end{equation}
...
\begin{theorem}
  ...
  to \(\tilde\beta\), \(\mathcal{B} \gets \mathcal{C}_k\) \mm{$\cB = \cC_k$ ?} is
  \[
    \partial =
    \begin{cases}
...
\end{theorem}
\begin{proof}
  Let \(f(\tilde\beta) = |\tilde\beta|\sum_{j \in \mathcal{C}_k}\lambda_{(j)^-} + \sum_{j \notin \mathcal{C}_k} \lambda_{(j)^-}|\beta_j|\).\mathurin{If $f$ is only a function of $\tilde \beta$, then the secodn term is constant, and since we are interested in subgradients, we should able to omit it?}
  \(g\) \mm{$f/g$ > $u$ or $v$ for subgradient?} is a subgradient of \(f\) at \(x\) if
  \begin{equation}
  ...
  \end{equation}
  for all \(y \in \mathbb{R}\).\mm{notation conflict with $y \in \bbR^n$}
...
\begin{equation}
  ...
  \begin{cases}
    ...
    \sign(a)|\tilde \beta_i|                                              & \text{if } \sum_{j= |\mathcal{C}_i| - |\mathcal{C}_k|}^{|\mathcal{C}_i|} \lambda^{\mathcal{C}_i}_j \leq |a| - |\tilde \beta_i| \leq \sum_{j=1}^{|\mathcal{C}_k|}\lambda^{\mathcal{C}_i}_j \\
...
\begin{algorithm}[tb]
  ...
  \Input{
      $X \in \mathbb{R}^{n\times p}, y\in \mathbb{R}^n, \beta\in \mathbb{R}^p, \lambda \in \mathbb{R}_{>0}^p, 
      \mathrm{\max\_{iter}} \in\mathbb{N}
      $}
  \caption{\texttt{Hybrid slopecd} (proposed)}\label{alg:sksparse}
  \For{$t = 1, \ldots,  \mathrm{\max\_{iter}}$}{
    \If{$t \, \mathrm{mod} \, 5 = 0$}{

      $\beta \leftarrow \mathrm{prox}_{J / L}(\beta - \frac{1}{L}\nabla f(\beta))$

      $C_1, \hdots, C_m \leftarrow \mathrm{get\_clusters}(\beta)$
      }
    \Else{
      \For{$b = 1, \hdots, m$}{
        $\mathcal{B} \leftarrow C_b$ 
        
        $s \leftarrow \mathrm{sign}(\beta_{\mathcal{B}})$

        $L_b = (X_{:, \mathcal{B}}s)^\top  X_{:, \mathcal{B}}s$

        $\tilde{\beta} = T(|\beta_{\mathcal{B}_1}| - \frac{1}{L_b} \nabla_\mathcal{B}f(\beta)^\top s, \frac{\lambda}{L_b})$

        $\beta_\mathcal{B} \leftarrow \tilde{\beta} s$
      }
    }
